const state = {
  ...
  stats: null, dashboard: null, importToken: null, actualMealUpload: null, weatherUpload: null, weatherOffset: 0, weatherHistoryOffset: 0, masterTab: 'menus', masterSelectionId: null, masterMenuDetailTab: 'recipe',
...
async function renderWeatherView(root){
  root.innerHTML=`<div class="weather-management">
    <section class="narrow-card"><h2>날씨정보 관리</h2><p>기상청 XLSX·XLS·CSV 관측자료를 분석한 뒤 저장합니다. 시간별 자료는 날짜·지점별 일자료로 자동 집계됩니다.</p>
    ...
    <section class="panel-section"><div class="panel-section-head"><h3>저장된 날씨자료</h3></div><div class="table-tools weather-filters"><input id="weather-start" type="date" title="시작일"><input id="weather-end" type="date" title="종료일"><input id="weather-station" placeholder="관측지점 검색"><input id="weather-min-temp" type="number" step="0.1" placeholder="평균기온 최소"><input id="weather-max-temp" type="number" step="0.1" placeholder="평균기온 최대"><select id="weather-rain"><option value="all">강수 전체</option><option value="yes">강수 있음</option><option value="no">강수 0mm</option></select><button class="secondary-button" id="weather-search">조회</button></div><div id="weather-records"><div class="empty-editor">불러오는 중입니다.</div></div></section>
  ...
  $('#weather-preview').addEventListener('click',previewWeatherFile);$('#weather-reset').addEventListener('click',()=>{state.weatherUpload=null;$('#weather-file').value='';$('#weather-preview-result').innerHTML='';});$('#weather-search').addEventListener('click',()=>{state.weatherOffset=0;loadWeatherRecords();});
  await Promise.all([loadWeatherRecords(),loadWeatherUploads()]);
}
...
function renderWeatherPreview(response){
  state.weatherUpload=response;const s=response.summary;const root=$('#weather-preview-result');const labels={observation_date:'관측일자',station_id:'지점번호',station_name:'지점명',avg_temp:'평균기온',min_temp:'최저기온',max_temp:'최고기온',precipitation:'강수량',avg_humidity:'평균습도',snow_depth:'적설',sunshine_hours:'일조시간'};
  root.innerHTML=`<div class="result-card"><h3>파일 분석 결과</h3><div class="stats-mini"><div class="stat-mini-card"><strong>파일</strong><span>${escapeHtml(response.filename)}</span></div><div class="stat-mini-card"><strong>자료 유형</strong><span>${s.aggregation_mode==='hourly_to_daily'?'시간별 → 일별 집계':'일별'}</span></div><div class="stat-mini-card"><strong>기간</strong><span>${s.date_from||'—'} ~ ${s.date_to||'—'}</span></div><div class="stat-mini-card"><strong>관측지점</strong><span>${numberText(s.stations.length)}개</span></div><div class="stat-mini-card"><strong>전체/정상</strong><span>${numberText(s.total_rows)} / ${numberText(s.valid_rows)}건</span></div><div class="stat-mini-card"><strong>신규/갱신/동일</strong><span>${numberText(s.inserted_rows)} / ${numberText(s.updated_rows)} / ${numberText(s.skipped_rows)}건</span></div><div class="stat-mini-card"><strong>오류/필드 확인</strong><span>${numberText(s.error_rows)} / ${numberText(s.warning_fields)}건</span></div></div>
  <div class="validation-result"><strong>자동 인식 컬럼</strong><div class="placeholder-list">${Object.entries(s.mapped_headers).map(([field,header])=>`<span class="placeholder-chip">${escapeHtml(header)} → ${labels[field]||field}</span>`).join('')}</div>${s.unknown_headers.length?`<p class="muted">사용하지 않은 컬럼: ${s.unknown_headers.map(escapeHtml).join(', ')}</p>`:''}</div>
